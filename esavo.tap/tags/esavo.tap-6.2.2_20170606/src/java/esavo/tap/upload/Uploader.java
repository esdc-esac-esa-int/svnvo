...
//			nbRowsLimit = -1;
	...
	public TAPSchema upload(UwsJobOwner owner, final UwsUploadResource[] loaders, DBConnection dbConn) throws TAPException, UwsException {
		long maxFileSize=0;

		TAPSchemaInfo tapSchemaInfo = service.getTapSchemaInfo(owner);
		TAPSchema uploadSchema = dbConn.loadSchema(tapSchemaInfo, TAPMetadataLoader.TAP_UPLOAD);
		InputStream votableIns = null;
		String tableName = null;
		nbRows = 0;
		try{
			for(UwsUploadResource loader : loaders){
				tableName = loader.getName();
...
//				}
	...
	*/
	...
	private TAPTable fetchTableMeta(final String tableName, final StarTable votable){
		TAPTable tapTable = new TAPTable(tableName, TAPTable.PUBLIC_TABLE);
