 ...
 */
public abstract class VOTableFormat implements OutputFormat {
	...
	private VOTableVersion votTableVersion = VOTableVersion.V13;
	...
	protected String shortMimeType = "votable";
	/**
	 ...
	 */
	public VOTableFormat(final TAPService service) throws NullPointerException {
		this(service, false);
	}
	 ...
	 * @throws NullPointerException	If the given service connection is <code>null</code>.
	 */
	public VOTableFormat(final TAPService service, final boolean logFormatReport) throws NullPointerException {
		if (service == null)
	 ...
	 * </ul>
	 * 
	 * @see esavo.tap.formatter.OutputFormat#writeResult(Object, OutputStream, TAPExecutionReport)
	 */
	public final long writeResult(final UwsJob job, final ResultSet queryResult, final OutputStream output, final TAPExecutionReport execReport) throws TAPException, InterruptedException {
		try{
			...
			out.println("\t\t\t<DATA>");
			out.flush();
			int nbRows = writeData(job, queryResult, columns, output, execReport);
			output.flush();
			out.println("\t\t\t</DATA>");
			out.println("\t\t</TABLE>");
			// OVERFLOW ?
			...
			if (logFormatReport)
				service.getFactory().getLogger().info("JOB "+execReport.jobID+" WRITTEN\tResult formatted (in VOTable ; "+nbRows+" rows ; "+columns.length+" columns) in "+(System.currentTimeMillis()-start)+" ms !");
	...
	protected void writeHeader(final PrintWriter output, final TAPExecutionReport execReport) throws IOException, TAPException {
		...
		if (votTableVersion != null)
			strBuf.append(" version=\"").append(SavotWriter.encodeAttribute(votTableVersion.getVersionNumber())).append('\"');
		if (xmlnsXsi != null)
		...
		String providerName = configuration.getProperty(TAPService.CONF_PROP_TAP_PROVIDER_NAME);
		String providerDesc = configuration.getProperty(TAPService.CONF_PROP_TAP_PROVIDER_DESC);
		output.println("\t\t<INFO name=\"PROVIDER\" value=\""+((providerName==null)?"":SavotWriter.encodeAttribute(providerName))+"\">"+
				((providerDesc==null)?"":SavotWriter.encodeElement(providerDesc))+"</INFO>");
		output.println("\t\t<INFO name=\"QUERY\"><![CDATA["+execReport.parameters.getQuery()+"]]></INFO>");
		output.println("\t\t<INFO name=\"PAGE\"><![CDATA["+execReport.parameters.getPage()+"]]></INFO>");
		output.println("\t\t<INFO name=\"PAGE_SIZE\"><![CDATA["+execReport.parameters.getPageSize()+"]]></INFO>");
		output.println("\t\t<INFO name=\"JOBID\"><![CDATA["+execReport.getJobId()+"]]></INFO>");
		output.println("\t\t<INFO name=\"JOBNAME\"><![CDATA["+execReport.getJobName()+"]]></INFO>");
	}
	 ...
	 */
	protected abstract DBColumn[] writeMetadata(UwsJob job, final ResultSet queryResult, final PrintWriter output, final TAPExecutionReport execReport) throws IOException, TAPException, InterruptedException;
	 ...
	 * @throws InterruptedException		If the given thread has been interrupted.
	 */
	protected abstract int writeData(UwsJob job, final ResultSet queryResult, final DBColumn[] selectedColumns, final OutputStream output, final TAPExecutionReport execReport) throws IOException, TAPException, InterruptedException;
