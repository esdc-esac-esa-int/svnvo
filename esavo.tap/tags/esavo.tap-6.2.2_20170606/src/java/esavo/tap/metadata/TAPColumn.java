...
public class TAPColumn implements DBColumn {
	...
	public TAPColumn(String name){
		this(name, false);
	}

	public TAPColumn(String name, boolean findNameOnly){
	...
	public final void setVotType(final VotType type){
		...
		String dbType = TAPTypes.getDBType(type);
		
		if(dbType==null){
			logger.error("Unknown VotType: "+type.toString()+" Setting default dbtype: varchar *");
		}
		
		this.datatype = TAPTypes.getTapType(dbType);
		this.size = type.arraysize;
		
		if (this.datatype == null)
			setDefaultType();
		
		//patch because STIL uses short for byte arrays
		if(TAPTypes.checkVarBinaryRequired(this.datatype, this.size)){
			this.datatype = TAPTypes.VARBINARY;
		}
	...
	public DBColumn copy(final String dbName, final String adqlName, final DBTable dbTable){
		return copy(dbName, adqlName, dbTable, true);
	}
	
	@Override
	public DBColumn copy(String dbName, String adqlName, DBTable dbTable, boolean findTableName) {
		String name;
		if(adqlName == null){
			name = this.adqlName;
		}else{
		...
		TAPColumn copy = new TAPColumn(name, findTableName);
