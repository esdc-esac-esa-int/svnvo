...
//	}
	...
	/** Get suitable filename (remove strange characters)
	 * 
	 */
	public static String getSuitableFilename(String filename){
		String formatted = filename.replaceAll("[\",;$@:/\\\\]", "_");
		return formatted;
	}
	
	/**
	 * Returns the suitable file name for a content disposition
	 * @param job job
	 * @param result job results
	 * @return the suitable file name for a content disposition
	 */
	public static String getSuitableContentDispositionFileName(UwsJob job, UwsJobResultMeta result){
    ...
    /**
     * Utility to write a data response header
     * @param response
     * @param dataid
     * @param mimeType
     * @param fileExtension
     ...
     * @param size
     */
    public static void writeDataResponseHeader(HttpServletResponse response, String dataid, String mimeType,
            String fileExtension, String contentEncoding, long size) {
        ...
        String extension = UwsUtils.getFileExtensionForMimeType(mimeType, contentEncoding);
        ...
        try{
        	// JCS magic
			String fileNameEncoded = URLEncoder.encode(dataid, "UTF-8");
	        filename = URLDecoder.decode(fileNameEncoded, "ISO8859_1");
        }catch(Exception e){}
