...
//	private Date dateToSet = null;
	...
	private Parameters getParameters(UwsActionRequest actionRequest) throws UwsException {
		...
		if(actionRequest.isPost()){
			...
			if(!actionRequest.hasHttpParameter(UwsHandlersUtils.PARAMETER_DESTRUCTION)){
				throw new UwsException(
						UwsOutputResponseHandler.BAD_REQUEST, 
						"Missing expected '"+UwsHandlersUtils.PARAMETER_DESTRUCTION+"' parameter for '"+ACTION_NAME+"' handler");
			}
			//check format
			String date = actionRequest.getHttpParameter(UwsHandlersUtils.PARAMETER_DESTRUCTION);
			try {
