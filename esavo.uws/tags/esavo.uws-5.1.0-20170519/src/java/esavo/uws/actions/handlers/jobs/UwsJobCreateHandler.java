 ...
 *
 */
public class UwsJobCreateHandler implements UwsActionHandler {
	...
	public boolean canHandle(String appid, UwsJobOwner owner, UwsActionRequest actionRequest) throws UwsException {
		...
		//if(actionRequest.getHttpParameter(UwsListJobsHandler))
...
//	}
	...
	private Parameters getParameters(UwsActionRequest actionRequest) throws UwsException {
		...
		if(actionRequest.hasHttpParameter(UwsHandlersUtils.PARAMETER_PHASE)){
			String sPhase = actionRequest.getHttpParameter(UwsHandlersUtils.PARAMETER_PHASE);
	...
	public void handle(UwsManager uwsManager, UwsJobOwner currentUser, UwsActionRequest actionRequest, HttpServletResponse response) throws UwsException {
		...
		if(!UwsUtils.checkValidList(listid, config)){
			throw new UwsException("List '"+listid+"' not supported. Available lists are: "+
					UwsUtils.dumpValidListIds(config));
		}
		if(actionParameters.priority < 0){
			actionParameters.priority = uwsManager.getFactory().getScheduler().getDefaultPriority(listid);
		}
		...
		String name = actionRequest.getHttpParameter(UwsHandlersUtils.PARAMETER_JOB_NAME);
		job.setName(name);
		...
		File uploadDir;
		if(UwsConfiguration.SYNC_LIST_ID.equals(listid)){
		...
		UwsJobParameters parameters= actionRequest.createJobParametersAsStrings(uploadDir, maxFileSize);
		
		// Add system parameters
		parameters.setParameter(UwsJobParameters.JOB_PARAMETER_UWS_VERSION, uwsManager.getFactory().getConfiguration().getUwsVersion());
		...
		if(UwsConfiguration.SYNC_LIST_ID.equals(listid)){
		...
		}else if (UwsConfiguration.ASYNC_LIST_ID.equals(listid)){
			//async.
			...
			throw new UwsException("List '"+listid+"' not supported. Available lists are: '"+
					UwsConfiguration.ASYNC_LIST_ID+"' and '"+UwsConfiguration.SYNC_LIST_ID+"'");
		}
	}
