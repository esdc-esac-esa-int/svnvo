...
public abstract class UwsShareAction implements UwsActionHandler {
	...
	public UwsShareAction(String id) {
		this.id = id;
	}
...
//	}
	...
	protected int getExistingResourceType(UwsActionRequest actionRequest) throws UwsException {
		String value = UwsHandlersUtils.getExistingParam(actionRequest, PARAM_RESOURCE_TYPE);
		try{
			return Integer.parseInt(value);
		}catch(NumberFormatException nfe){
			throw new UwsException(UwsOutputResponseHandler.BAD_REQUEST, 
					"Invalid required parameter '"+PARAM_RESOURCE_TYPE+"' value: " + value + ". Expected an integer value.");
		}
	}
	
	protected UwsShareType getExistingShareType(UwsActionRequest actionRequest) throws UwsException {
		String value = UwsHandlersUtils.getExistingParam(actionRequest, PARAM_SHARE_TYPE);
		UwsShareType shareType;
		try{
			shareType = UwsShareType.valueOf(value);
		}catch(IllegalArgumentException e){
			throw new UwsException(UwsOutputResponseHandler.BAD_REQUEST, 
					"Invalid required parameter '"+PARAM_SHARE_TYPE+"' value: " + value + 
					". " + getValidValuesForShareType());
		}
		if(shareType == UwsShareType.All){
			throw new UwsException(UwsOutputResponseHandler.BAD_REQUEST, 
					"Invalid required parameter '"+PARAM_SHARE_TYPE+"' value: " + shareType + 
					". " + getValidValuesForShareType());
		}
		return shareType;
	}
	...
	protected UwsShareMode getExistingShareMode(UwsActionRequest actionRequest) throws UwsException {
		String value = UwsHandlersUtils.getExistingParam(actionRequest, PARAM_SHARE_MODE);
		UwsShareMode shareMode;
		try{
			shareMode = UwsShareMode.valueOf(value);
		}catch(IllegalArgumentException e){
			throw new UwsException(UwsOutputResponseHandler.BAD_REQUEST, 
					"Invalid required parameter '"+PARAM_SHARE_MODE+"' value: " + value + 
