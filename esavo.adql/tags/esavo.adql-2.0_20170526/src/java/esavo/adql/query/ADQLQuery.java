...
import esavo.adql.db.DefaultDBColumn;
import esavo.adql.db.SearchColumnList;
...
import esavo.adql.search.ISearchHandler;
	...
	public DBColumn[] getResultingColumns(){
		...
		for(SelectItem item : select){
			...
			if (item instanceof SelectAllColumns){
				SelectAllColumns selectAllColumnsItem = (SelectAllColumns) item;
				try{
					SearchColumnList allColumns = from.getDBColumns();
					
					for(DBColumn col:allColumns){
						
						// IF no table is specified in *, add all columns
						if(selectAllColumnsItem.getAdqlTable()==null){
							columns.add(col);
						}else{
							//DBTable fromDbTable = col.getTable();
							//ADQLTable selectAdqlTable = ((SelectAllColumns) item).getAdqlTable();
	
							String fromTable = col.getTable().getADQLName();
							String columnTable = selectAllColumnsItem.getAdqlTable().getName();
							
							//System.out.println(fromTable+"   "+fromTable);
							
							if(fromTable.equals(columnTable)){
								columns.add(col);
							}
						}
					}
				...
				if (item.hasAlias()){
					if (operand instanceof ADQLColumn && ((ADQLColumn)operand).getDBLink() != null){
						...
						col = col.copy(col.getDBName(), item.getAlias(), col.getTable(), false);
					}else
