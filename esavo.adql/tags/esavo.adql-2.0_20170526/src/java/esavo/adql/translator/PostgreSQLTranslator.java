...
public class PostgreSQLTranslator implements ADQLTranslator {
	...
	public String translate(ADQLQuery query) throws TranslationException{
		...
		//Disable sequential scan for this query when the query is geometrical
		String sqlString = sql.toString();
		/*
		if(sql.toString().contains("q3c_join")) {
			sqlString = "set enable_seqscan = off;" + sqlString;
		} else {
			sqlString = "set enable_seqscan = on;" + sqlString;			
		}
		*/
		return sqlString;
	}
	...
	public String translate(SelectItem item) throws TranslationException{
		...
		if (item.hasAlias()){
			...
			String id = getSuitableAlias(item.getName());
			item.setAlias(id);
			translation.append(" AS ").append(id);
		}

		return translation.toString();
	}
	
	private String getSuitableAlias(String id){
		//get first char: 
		//if numeric or number, return it
		//Otherwise, return exprN
		if (id == null) {
			return id;
		}
		int size = id.length();
	...
	public String translate(MathFunction fct) throws TranslationException{
		switch(fct.getType()){
			case ABS:
				return "abs(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case CEILING:
				return "ceil(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) + "::numeric" : "") + ")";
			case DEGREES:
				return "degrees(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case EXP:
				return "exp(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case FLOOR:
				return "floor(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case LOG:
				return "ln(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case LOG10:
				return "log(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case MOD:
				return "mod(" + ((fct.getNbParameters() >= 2) ? (translate(fct.getParameter(0)) + ", " + translate(fct.getParameter(1))) : "") + ")";
			case PI:
				return "pi()";	
			case RAND:
				return "random()";
			case ROUND:
				return "round(" + ((fct.getNbParameters() >= 2) ? (translate(fct.getParameter(0)) + "::numeric, " + translate(fct.getParameter(1))) : "") + ")";
			case SQRT:
				return "sqrt(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case TRUNCATE:
				return "trunc(" + ((fct.getNbParameters() >= 2) ? (translate(fct.getParameter(0)) + "::numeric, " + translate(fct.getParameter(1))) : "") + ")";
			case POWER:
				return "power(" + ((fct.getNbParameters() >= 2) ? (translate(fct.getParameter(0)) + ", " + translate(fct.getParameter(1))) : "") + ")";
			case ACOS:
				return  "acos(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case ASIN:
				return "asin(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case ATAN:
				return "atan(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case ATAN2:
				return "atan2(" + ((fct.getNbParameters() >= 2) ? (translate(fct.getParameter(0)) + ", " + translate(fct.getParameter(1))) : "") + ")";
			case COS:
				return  "cos(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case SIN:
				return "sin(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			case TAN:
				return "tan(" + ((fct.getNbParameters() >= 1) ? translate(fct.getParameter(0)) : "") + ")";
			default:
				return getDefaultADQLFunction(fct);
		}
	}
	...
	public String translate(GeometryFunction fct) throws TranslationException{
		...
		else if (fct instanceof ResolveTargetFunction)
			return translate((ResolveTargetFunction)fct);	
	...
	public String translate(ResolveTargetFunction resolveTarget) throws TranslationException{
		return getDefaultGeometryFunction(resolveTarget);
	}
